...
Функция ПолучитьТаблицуСерверов(СГруппировкойОрганизация = Ложь, ОтборОрганизация = Неопределено) Экспорт
	
	Запрос = Новый Запрос; 	
	Запрос.Текст = "ВЫБРАТЬ
	               |	РасположениеФирм.Сервер КАК Сервер,
	               |	РасположениеФирм.ИмяБазы КАК База,"+?(СГруппировкойОрганизация," РасположениеФирм.Организация,","")+"
	               |	ИСТИНА КАК Пометка,
	               |	РасположениеФирм.Логин КАК Пользователь,
	               |	РасположениеФирм.Пароль КАК Пароль
	               |ИЗ
	               |	РегистрСведений.РасположениеФирм КАК РасположениеФирм
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	РасположениеФирм.Сервер,
	               |	РасположениеФирм.ИмяБазы,"+?(СГруппировкойОрганизация," РасположениеФирм.Организация,","")+"
	               |	РасположениеФирм.Логин,
	               |	РасположениеФирм.Пароль";
	
	Если НЕ ОтборОрганизация = Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "РегистрСведений.РасположениеФирм КАК РасположениеФирм", "РегистрСведений.РасположениеФирм КАК РасположениеФирм"+Символы.ПС+"ГДЕ РасположениеФирм.Организация В (&Организация)");
