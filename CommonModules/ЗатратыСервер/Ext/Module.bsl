...
Процедура СформироватьЭтотОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РасположениеФирм.СрезПоследних(&Период, ) КАК РасположениеФирмСрезПоследних
	|		ПО Организации.Ссылка = РасположениеФирмСрезПоследних.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязьОрганизаций8ИСчетов7.СрезПоследних(&Период, ) КАК СвязьОрганизаций8ИСчетов7
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "#Условие", "И Организации.Ссылка В ИЕРАРХИИ (&Ссылка)");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл	
		...
		Попытка
			...
			Если НЕ ПараметрыОтчета.ТолькоДанныеВыгрузки Тогда 
				СформироватьТабличноеПредставление(Прокси, ПараметрыОтчета, ВыборкаДетальныеЗаписи.Ссылка, ТабличныйДокумент, Макет, Группа, ВыборкаДетальныеЗаписи, НомерЗаписи);
			КонецЕсли;	
			
			Если НЕ ПараметрыОтчета.НеФормироватьДанныеДляВыгрузки Тогда
	...
	ПарамФайлов = Новый Структура("ТаблДокумент,МассивОрганизацийСОшибками,Данные"
		, ТабличныйДокумент
		, МассивОрганизацийСОшибками
		, Неопределено//ПотокВПамяти.ЗакрытьИПолучитьДвоичныеДанные()
	);
...
Функция СформироватьТабличноеПредставление(Прокси, ПараметрыОтчета, Организация, ТабличныйДокумент, Макет, Группа, ДанныеВыборки, НомерПП) Экспорт
	
	/////////////////////////////////////////////////////////
	// Выведем шапку
	
	СведенияОПокупателе = Организация.Наименование;
	СведенияОПокупателе = "" + НомерПП + ". " + СведенияОПокупателе + ", ("+ ?(Группа.Количество() > 0, СокрЛП(Группа[0].Счет7), "")+", "+ДанныеВыборки.Бухгалтер+")";
...
Функция ВыпискаПроверитьСтроку(Данные, ДокВыписка, ДопПараметр, ДелатьЗеркальнуюОперацию = Ложь) Экспорт
	...
	ИначеЕсли (СтрНайти(Содержание, "Оплата поставщикам") > 0 ИЛИ СтрНайти(Содержание, "Прочие расходы") > 0 ИЛИ СтрНайти(Содержание, "СБП") > 0 ИЛИ ПустаяСтрока(Содержание)) 
		...
		И ДокВыписка.КоррСчет = "60.1"
		И ДокВыписка.Субконто1 = "ЕМ" И СтрНайти(СокрЛП(ДокВыписка.Субконто2), "затр") > 0 Тогда
		Возврат Истина;		
	...
	ИначеЕсли (СтрНайти(Содержание, "Возврат от поставщик") > 0) 
		...
		И (ДокВыписка.ВидДвижения = "Возврат от поставщика" ИЛИ ДокВыписка.ВидДвижения = "Прочие поступления") 
		И ДокВыписка.КоррСчет = "60.1"
		И ДокВыписка.Субконто1 = "ЕМ" И СтрНайти(СокрЛП(ДокВыписка.Субконто2), "затр") > 0
		И Истина Тогда
		Возврат Истина;			
	...
	ИначеЕсли ИСТИНА//(СтрНайти(Содержание, "Прочие налоги и сборы") > 0 ИЛИ СтрНайти(Содержание, "Налог на прибыль") > 0 ИЛИ СтрНайти(Содержание, "Штрафы госорганов") > 0 ИЛИ СтрНайти(Содержание, "Прочие расходы") > 0) 
		...
		И (ДокВыписка.ВидДвижения = "Оплата поставщику" ИЛИ ДокВыписка.ВидДвижения = "Прочие выплаты")
		И ДокВыписка.КоррСчет = "60.1"
		И ДокВыписка.Субконто1 = "ЕМ" И СтрНайти(СокрЛП(ДокВыписка.Субконто2), "ЗП/налоги") > 0 
		И Истина Тогда
		Возврат Истина;		
...
Функция Услуги(Прокси, ПараметрыОтчета, Организация, ТабличныйДокумент, Макет)
	...
	Запрос.ТекстЗапроса = 
		...
		|			Справочник.Контрагенты КАК Контрагенты
		|		ГДЕ
		|			(Контрагенты.Наименование В (&ГруппаКонтрагентов)
		|					ИЛИ Контрагенты.Родитель.Наименование В (&ГруппаКонтрагентов)
		|					ИЛИ Контрагенты.Родитель.Родитель.Наименование В (&ГруппаКонтрагентов)
		|					ИЛИ Контрагенты.Родитель.Родитель.Родитель.Наименование В (&ГруппаКонтрагентов))
		|		;
		|			ВЫБРАТЬ	
		|			Контрагенты.Ссылка КАК Ссылка
		|		ПОМЕСТИТЬ ВТЗаймы
		...
		|			Справочник.Контрагенты КАК Контрагенты
		|		ГДЕ
		|			(Контрагенты.Наименование ПОДОБНО ""ЗАЙМЫ%""
		|					ИЛИ Контрагенты.Родитель.Наименование ПОДОБНО ""ЗАЙМЫ%""
		|					ИЛИ Контрагенты.Родитель.Родитель.Наименование ПОДОБНО ""ЗАЙМЫ%""
		|					ИЛИ Контрагенты.Родитель.Родитель.Родитель.Наименование ПОДОБНО ""ЗАЙМЫ%"")
		|		;
		|
		|		
		|		////////////////////////////////////////////////////////////////////////////////
...
Функция Займы(Прокси, ПараметрыОтчета, Организация, ТабличныйДокумент, Макет)
	...
	Запрос.Параметры.Вставить("ДатаОкончания", Новый Граница(ПараметрыОтчета.КонецПериода, ВидГраницы.Включая));
	Запрос.Параметры.Вставить("Организация", Организация.ИНН);	
	Запрос.Параметры.Вставить("ГруппаКонтрагентов", "ЗАЙМЫ");	
	Запрос.Параметры.Вставить("Счет", "58");		//ПланыСчетов.Хозрасчетный.РасчетыСПоставщикамиИПодрядчиками);
	Запрос.Параметры.Вставить("КорСчет", "51");		//      ПланыСчетов.Хозрасчетный.РасчетныеСчета);
	Запрос.ТекстЗапроса = 
		"ВЫБРАТЬ
		|	Хозрасчетный.Ссылка КАК Счет
	...
	Если НЕ Ответ.Ошибка Тогда
		Если Ответ.Результат.Количество() > 0 Тогда
			ТаблицаУслуг = Ответ.Результат;
			ОбластьМакета = Макет.ПолучитьОбласть("ИтогиЗаймы");
			ИтогоЗаймы = ТаблицаУслуг.Итог("СуммаУслуг");
			ОбластьМакета.Параметры.СуммаУслуг = ИтогоЗаймы;
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			ОбластьМакета = Макет.ПолучитьОбласть("ЗаймыДетально");
			Для Каждого Стр Из ТаблицаУслуг Цикл
				ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, Стр);
