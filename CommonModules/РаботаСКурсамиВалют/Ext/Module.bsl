...
Процедура ЗагрузитьАктуальныйКурсНБУ(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	
	Если Метаданные.Обработки.Найти("ЗагрузкаКурсовВалютНБУ") <> Неопределено Тогда 
		ТекОбъект = Обработки.ЗагрузкаКурсовВалютНБУ.Создать();
		ТекОбъект.ЗавантажитиКурсиУсіхВалют();
	КонецЕсли;
